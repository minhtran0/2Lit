	...
	if (isset($_SESSION['userid'])) {
		if (isset($_POST['submit'])) {
			...
			if ($success) {
				...
				$query = "SELECT user_id, username FROM lit_comment WHERE post_id = '$postid'";
				...
				while($row = $result->fetch_assoc()) {
					...
					$conn->query($query);
					$query3 = "INSERT INTO 
									lit_notification (user_id, commenter_user_id, post_id, datetime_posted) 
								VALUES
									('".$row['user_id']."', '".$_SESSION['userid']."', '$postid', NOW())";
				...
				if (!$op_posted) {
					...
					$conn->query($query);
					$query3 = "INSERT INTO 
									lit_notification (user_id, post_id, datetime_posted) 
								VALUES
									('$op_userid', '$postid', NOW())";
				}
